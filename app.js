(function(){
  ...
  //  マッチング本体（関数化）
  // =========================
  function computeMatches(lines, master) {
    const uniq      = new Set();
    const matched   = new Set();
    const unmatched = new Set();

    // master: [{ name, loose }] を想定
    const masterLooseList = master.map(m => m.loose);

    for (const raw of lines) {
      const norm  = NameNormalizer.normalizeName(raw);
      const loose = NameNormalizer.makeLooseKey(norm);
      ...
      for (const m of master) {
        if (m.loose.includes(loose) || loose.includes(m.loose)) { best = m; break; }
      }
      if (best) { matched.add(best.name); continue; }

      // 3) レーベンシュタイン距離（長さ比例しきい値）
      let minD = Infinity, near = null;
      for (const m of master) {
        const d = levenshtein(loose, m.loose);
        if (d < minD) { minD = d; near = m; }
      }
      const baseLen   = Math.min(loose.length, (near?.loose.length || 0));
      const threshold = Math.max(2, Math.floor(baseLen * 0.12)); // 12% 以内ならOK
  ...
  //  エクスポート（ボタン）
  // =========================
  const matchedTA = document.querySelector('#matchedTA'); // ← テキストエリアのID想定
  document.querySelector('#btnExportCsv')?.addEventListener('click', () => {
    const rows = (matchedTA?.value || '')
